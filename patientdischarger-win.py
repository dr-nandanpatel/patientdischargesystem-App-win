...
class BackEnd:
    ...
    def PatientDischarger(self):
        ...
        if os.path.isfile(filepath):
            status_done_label = tk.Label(root, text='Patient report created successfully!', fg='green')
            canvas_main.create_window(300, 650, window=status_done_label)
            return
        if not os.path.isfile(filepath):
            status_done_label = tk.Label(root, text='Could not create patient report! Please try again...', fg='red')
            canvas_main.create_window(300, 650, window=status_done_label)
            return
...
if __name__ == "__main__":
    ...
    fontStyle = tkFont.Font(family="Calibri", size=23)
